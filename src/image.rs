...
const WIDTH: u32 = 128;
const HEIGHT: u32 = 72;
const MAX_HEROES_BY_PLAYER: u32 = 8;
const MARGE: u32 = 1;
const MAX_COLS: u32 = 2;
const NU_WITDH: u32 = 18;
const NU_HEIGHT: u32 = 30;
...
fn draw_player(image: &mut RgbaImage, player: &Player, absolute_x: u32, absolute_y: u32) {
    ...
    .of_size(
        if player.expert.len() as u32 > MAX_HEROES_BY_PLAYER {
    ...
    draw_filled_rect_mut(image, rect, Rgb([51u8, 153u8, 102u8]).to_rgba());
    ...
    if !player.op.is_empty() {
        draw_filled_rect_mut(image, rect, Rgb([255u8, 102u8, 0u8]).to_rgba());
    }
